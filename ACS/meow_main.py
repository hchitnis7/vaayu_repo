...
def checkPath(checkpointss):
    ...
    while True:
        ...
        y = [lis[1] for lis in checkpoints]
        y = y[::-1]
        for i in range(len(x)):
            ckp_alt.append(float((cp.findaltitude(x[i], 0 , destination_lat, 0))))
        print("The landing trajectory is: \n")
        print(ckp_alt)
        ptpr.plot_points(y, ckp_alt)
        ...
        for i in range(1, len(copied_checkpoint)):
            if plane_coordinates[1] > checkpoints[i][1]:
                ...
                checkpointss.pop(i)
                break
            if abs(plane_altitude - curved_altitude) > 2:
                print('New landing trajectory')
                checkpoints = cp.create_checkpoint(plane_coordinates[0], plane_coordinates[1], destination_lat, destination_log, plane_length)
                #x = [lis[0] for lis in checkpoints]
                #y = [lis[1] for lis in checkpoints]
                #y = y[::-1]
                #for i in range(len(x)):
                    #ckp_alt.append(float((cp.findaltitude(x[i], destination_lat))))
                #print("The landing trajectory is: \n")
                #print(ckp_alt)
                #ptpr.plot_points(y, ckp_alt)
                print('Check Point Altitude is : ', curved_altitude)
